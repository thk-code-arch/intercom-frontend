    ...
    <div class="z-50 w-11/12 mx-auto overflow-y-auto bg-white rounded modal-container lg:max-w-4xl md:max-w-md">
      ...
      <div class="px-6 py-4 text-left modal-content">
        ...
        <div class="flex flex-col" v-if="!selType">
          ...
          <button  v-on:click="selType = 'feature'" class="px-4 py-2 m-3 font-semibold text-blue-700 bg-transparent border border-blue-500 rounded hover:bg-blue-500 hover:text-white hover:border-transparent">
            Feature Request - Suggest an idea for this project
          </button>
          <button v-on:click="selType = 'bug'" class="px-4 py-2 m-3 font-semibold text-blue-700 bg-transparent border border-blue-500 rounded hover:bg-blue-500 hover:text-white hover:border-transparent">
            Report a bug - Create a report to help us improve
          </button>

        </div>
            <FormulateForm v-if="selType =='feature'" @submit="sendBugReport(reportFeature,feature.title,'Feature Request')">
                <FormulateInput
                  type="text"
                  name="Title"
            ...
            <FormulateForm v-if="selType =='bug'" @submit="sendBugReport(reportBug,bug.title,'Bug')">
                <FormulateInput
                  type="text"
                  name="Title"
                  v-model="bug.title"
            ...
            <div v-if="response">
              <span>Thanks for submitting, Incident can be found at {{response.html_url}} (Access is restricted)</span>
            </div>
        <!--Footer-->
        <div v-if="selType =='sent'" class="flex justify-end pt-2">
          <button v-on:click="exitButton()" class="p-3 px-4 text-white rounded-lg bg-codearch-500 modal-close hover:bg-codearch-300">Close</button>
        </div>
      </div>
    </div>
  </div>
...
${yourContext}`
  ...
  mounted(){
      ...
      this.context.username = this.$store.state.auth.user.username;
